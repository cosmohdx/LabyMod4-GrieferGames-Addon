...
public class GGServerMessageListener {
  ...
  public void onServerMessage(NetworkPayloadEvent event) {
    ...
    if(event.side() == Side.RECEIVE &&
      ...
      if(messageKey.equals("user_subtitle") && message.isJsonArray() && !message.getAsJsonArray().isEmpty()) {
        JsonObject subtitleData = message.getAsJsonArray().get(0).getAsJsonObject();
        if (subtitleData.has("text") && subtitleData.has("targetId")) {
          String text = Laby.labyAPI().minecraft().componentMapper()
              .translateColorCodes(subtitleData.get("text").getAsString());
          Subtitle.create(
              UUID.fromString(subtitleData.get("targetId").getAsString()),
              ServerAPIComponent.text(text),
              1.2);
        }
      }
    }
